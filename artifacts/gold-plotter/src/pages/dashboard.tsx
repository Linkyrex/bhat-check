...
import { TrendingUp, TrendingDown, Clock, Activity, BarChart3, AlertCircle, ExternalLink } from "lucide-react";
...
const formatTHBChange = (val: number) =>
  `${val >= 0 ? "+" : ""}${new Intl.NumberFormat("th-TH", { style: "currency", currency: "THB", maximumFractionDigits: 0 }).format(val)}`;
    ...
    <div className="min-h-screen relative z-10 w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
      ...
      <main>
          ...
          <div>
            ...
            {isLoadingCurrent || !currentData ? (
              ...
              <div className="text-rose-500 flex items-center gap-2 p-4 bg-rose-500/10 rounded-xl border border-rose-500/20">
                <AlertCircle className="w-5 h-5" /> Failed to load current price
              </div>
            ) : (
              <motion.div
              ...
              >
                ...
                <span className={cn(
                  ...
                  {formatTHBChange(currentData.change)}
                  <span className="ml-1.5 opacity-80 text-lg">
            ...
            {ranges.map((r) => (
              <button
                ...
                className={cn(
                  ...
                  range === r
                    ? "bg-gradient-to-b from-primary/90 to-primary text-primary-foreground shadow-lg shadow-primary/30"
        ...
        <motion.div
          ...
          className="w-full bg-card/40 border border-border/60 rounded-[32px] p-5 sm:p-8 lg:p-10 backdrop-blur-2xl mb-3 relative group shadow-2xl"
        >
        ...
        <div className="flex justify-end mb-8 px-2">
          <a
            ...
            className="inline-flex items-center gap-1.5 text-xs text-muted-foreground/70 hover:text-primary transition-colors duration-200"
          >
            <ExternalLink className="w-3 h-3" />
            Source: สมาคมค้าทองคำ (Gold Traders Association of Thailand)
          </a>
        </div>
        ...
        {historicalData && !isLoadingHistorical && (
          ...
          >
            <StatCard title="Period High" value={formatTHB(historicalData.maxPrice)} icon={TrendingUp} trend="up" />
            <StatCard title="Period Low" value={formatTHB(historicalData.minPrice)} icon={TrendingDown} trend="down" />
            <StatCard
            ...
            />
            <StatCard
              title="vs Period High"
              value={currentData
                ? `${(((currentData.price - historicalData.maxPrice) / historicalData.maxPrice) * 100).toFixed(2)}%`
                : "-"}
              icon={Clock}
              trend="neutral"
            />
